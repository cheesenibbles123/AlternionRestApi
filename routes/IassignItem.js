...
module.exports = {
	...
	route: (router) => {
		router.get('/', async (req, res) => {
		  if (req.query.steamID && req.query.key && req.query.type && req.query.id){
		    	...
		    	let steamID = parseInt(req.query.steamID);
		     	if (!isNaN(steamID)){
					...
					switch (req.query.type.toLowerCase()){
		     		...
		     		if (table1Name != "NA"){
						db.connectionPool.query(`SELECT Team_ID FROM User WHERE Steam_ID=${req.query.steamID}`, (err,userRow) => {
							db.connectionPool.query(`SELECT ${table1Name}.Name, ${table1Name}.Display_Name, ${table1Name}.ID, ${table1Name}.Value FROM ${table2Name} INNER JOIN User ON User_ID = User.ID INNER JOIN ${table1Name} ON ${table2Field} = ${table1Name}.ID WHERE User.Steam_ID='${req.query.steamID}'`, (err, rows) => {
								db.connectionPool.query(`SELECT ${table1Name}.Name, ${table1Name}.Team_ID, ${table1Name}.Display_Name, ${table1Name}.ID, ${table1Name}.Value FROM ${table1Name} WHERE Limited!=True OR ( Team_ID=${userRow[0].Team_ID} AND IF ( ${userRow[0].Team_ID} != 0, 1, 0) = 1 )`, (err, rows2) => {
									let found = false;
									if (rows){
									...
									if (found){
										responses.returnSuccess(res,`Successfully assigned item.`);
									}else{
